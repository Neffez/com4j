...
abstract class MethodBinder {
    ...
    /**
     * If the return value is a part of the parameter (such as in the VTBL binding), this value points to that index.
     * Otherwise -1.
     * <p>
    ...
    protected MethodBinder(final Generator g, final IMethod method) throws BindingException {
        this.g = g;
        this.method = method;

        final int len = method.getParamCount();
    ...
    protected IType getReturnTypeBinding() throws BindingException {
        ...
        final IParam p = params[retParam];
        final IType t = p.getType();
        if (t == null) {
    ...
    public void declare(final IndentingWriter o) throws BindingException {
        final Parameter[] defaultParams = generateDefaults();
        final boolean[] useDefault = new boolean[defaultParams.length];
    ...
    class Parameter {
        ...
        /**
         * {@link #javaTypeName} can be a {@link Type}, not just {@link Class} (for example,
         * {@code Holder&lt;Something>},
        ...
        public String toClassName() {
            final int idx = javaTypeName.indexOf('<');
            if (idx >= 0) {
                return javaTypeName.substring(0, idx);
            } else {
                return javaTypeName;
            }
        }
    ...
    private Parameter[] generateDefaults() {
        final Parameter[] defParam = new Parameter[params.length];
        for (int i = 0; i < params.length; i++) {
    ...
    private void declareJavaDoc(final IndentingWriter o, final Parameter[] defaultParam, final boolean[] useDefault) {
        // JavaDoc
        o.beginJavaDocMode();
        final String helpString = method.getHelpString();
        if (helpString != null) {
            o.println("<p>");
    ...
    private void declareWithDefaults(final IndentingWriter o, final Parameter[] defaultParam,
            final boolean[] useDefault) throws BindingException {

        declareJavaDoc(o, defaultParam, useDefault);

        annotate(o);

        final int dispId = method.getDispId();
        if (dispId == 0) {
        ...
        if (useDefault != null) {
            ...
            final String defaultsAnnotation = "@UseDefaultValues("
    ...
    private static String join(final Collection<?> args, final String delim) {
    ...
    protected final void declareMethodName(final IndentingWriter o) {
        String methodName = method.getName();
        if (g.renameGetterAndSetters) {
            final String methodStart = methodName.length() > 3 ? methodName.substring(0, 3) : "";
    ...
    protected final void declareParameters(final IndentingWriter o, final boolean[] useDefaults)
     ...
     */
    private void declare(final IndentingWriter o, final IParam p) throws BindingException {
        final TypeBinding vb = TypeBinding.bind(g, p.getType(), p.getName());
        ...
        final Variant defValue = p.getDefaultValue();
        if (defValue != null) {
            try {
    ...
    protected void declareParamName(final IndentingWriter o, final IParam p) {
    ...
    /**
     * Returns true if this method generator needs to generate {@link MarshalAs}. This is only necessary for VTBL
     * method.
     */
     ...
     */
    protected final void declareReturnType(final IndentingWriter o, final List<IType> intermediates,
            final boolean usesDefaltValues) throws BindingException {
        ...
        if (returnType == null && intermediates == null) {
            ...
            final TypeBinding retBinding = TypeBinding.bind(g, returnType, null);

            // add @ReturnValue if necessary
            if (!retBinding.isDefault && needsMarshalAs()
                    || retParam != -1 && (params[retParam].isIn() || retParam != params.length - 1)
                    || intermediates != null || usesDefaltValues) {
                o.print("@ReturnValue(");
                o.beginCommaMode();
                if (!retBinding.isDefault && needsMarshalAs()) {
                    o.comma();
                ...
                if (intermediates != null) {
                    ...
                    o.beginCommaMode();
                    for (final IType im : intermediates) {
                        final TypeBinding vb = TypeBinding.bind(g, im, null);
                        o.comma();
                        o.print(vb.javaType);
                        o.print(".class");
                    }
    ...
    private String camelize(final String s) {
        int idx = 0;
    ...
    static boolean isEnum(final IMethod m) {
    ...
    protected final IType getDispInterfaceReturnType() {
        final IType r = method.getReturnType();

        // if the return type is HRESULT, bind it to 'void'.
        // dispinterfaces defined by C++ often uses HRESULT
        // as the return value
        final IPrimitiveType pt = r.queryInterface(IPrimitiveType.class);
