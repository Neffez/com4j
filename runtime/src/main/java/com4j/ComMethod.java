...
abstract class ComMethod {
    ...
    /**
     * The parameter indexes of default parameters. Each position in this array corresponds to a position in
     * {@link #defaultParameters}
     *
     * <p>
     * <h3>Example</h3>
     ...
     * </pre>
     *
     * In this example the second parameter of the {@link ComMethod} has a default value of <code>new Integer(3)</code>
     * and the
     ...
     */
    public ComMethod(final Method method) {
        this.method = method;

        // generate default values
        final UseDefaultValues defValues = method.getAnnotation(UseDefaultValues.class);
        if (defValues != null) {
        ...
        final int paramLen = pa.length;

        // retrieve information about the return value
        final ReturnValue rt = method.getAnnotation(ReturnValue.class);
        if (rt != null) {
            if (rt.index() == -1) {
                returnIndex = pa.length;
            } else {
        ...
        final Class<?>[] paramTypesLocal = method.getParameterTypes();
        final Type[] genericParamTypesLocal = method.getGenericParameterTypes();
    ...
    protected void messageParameters(final Object[] args) {
    ...
    static NativeType getDefaultConversion(final Type t) {
        if (t instanceof Class) {
            ...
            final NativeType r = defaultConversions.get(c);
            if (r != null) {
                return r;
            }
        ...
        if (t instanceof ParameterizedType) {
            ...
            if (p.getRawType() == Holder.class) {
                // let p=Holder<V>
                final Type v = p.getActualTypeArguments()[0];
                final Class<?> c = v instanceof Class ? (Class<?>) v : null;
                if (c != null) {
                    if (Com4jObject.class.isAssignableFrom(c)) {
     ...
     */
    protected void generateDefaultParameters(final UseDefaultValues defValues) {
        final int count = defValues.optParamIndex().length;
        final NativeType[] nt = defValues.nativeType();
        final Variant.Type[] vt = defValues.variantType();
        final String[] literal = defValues.literal();
            ...
            switch (nt[i]) {
                ...
                case VARIANT:
                    ...
                    switch (vt[i]) {
                        ...
                        // case VT_CY: ...
                        default:
                            throw new UnsupportedOperationException("Don't know how to parse literal " + literal[i]
                                    + " to an Java Object corresponding to Variant.Type." + vt[i].name());
                    }
                    v.setType(vt[i]);
                ...
                default:
                    throw new UnsupportedOperationException("Don't know how to parse literal " + literal[i]
                            + " to an Java Object corresponding to NativeType." + nt[i].name());
            }
        }
    }
}
