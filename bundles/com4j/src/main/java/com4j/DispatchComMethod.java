...
final class DispatchComMethod extends ComMethod {
    ...
    DispatchComMethod(final Method m) {
        super(m);

        final DISPID id = m.getAnnotation(DISPID.class);
        if (id == null) {
    ...
    private int getFlag() {
        final PropGet get = method.getAnnotation(PropGet.class);
        final PropPut put = method.getAnnotation(PropPut.class);
        if (get != null && put != null) {
            throw new IllegalAnnotationException(
                    "@PropPut and @PropGet are mutually exclusive: " + method.toGenericString());
        }
        if (get != null) {
    ...
    Object invoke(final long ptr, final Object[] args) {
        ...
        final Variant v = Native.invokeDispatch(ptr, dispId, flag, args);
        if (v == null) {
            return null;
        }
