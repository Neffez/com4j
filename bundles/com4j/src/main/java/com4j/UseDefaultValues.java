...
public @interface UseDefaultValues {
    /**
     * Returns an array of indices, that maps the parameter of the java interface to the parameter index of the COM
     * interface <br>
     ...
     * </pre>
     *
     * The first function would not need a {@link UseDefaultValues} annotation at all. The second function needs an
     * annotation with a parameter mapping [0, 1, 3]
     * and the third function needs the mapping [0, 3] <br>
     * The mapping wouldn't be necessary, if we consider that there is at most one <code>[lcid]</code> parameter and
     * that this parameter always needs to be at the
     * end of the optional parameters. But consider the case, where we have an list of optional parameters with default
     * values and Com4j is not able to handle an
     * optional parameter in the middle of list. Using parameter index mapping makes it possible to generate Java
    ...
    /**
     * A list of {@link Variant.Type}s used to generate the default parameters. The entry in this list is only needed in
     * the case that the corresponding
